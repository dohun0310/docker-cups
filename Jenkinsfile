pipeline {
  ...
  stages {
    ...
    stage("Build") {
      steps {
        script {
          sh "docker buildx build --platform linux/amd64,linux/arm64,linux/arm/v7 -t ${DOCKER_IMAGE_STORAGE}/${DOCKER_IMAGE_NAME}:${DOCKER_IMAGE_TAG} -t ${DOCKER_IMAGE_STORAGE}/${DOCKER_IMAGE_NAME}:${VERSION} --output type=docker,dest=/tmp/${DOCKER_IMAGE_NAME}.tar ."
